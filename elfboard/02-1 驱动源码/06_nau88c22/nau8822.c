	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	for (i = 0; i < params->max / sizeof(u16); i++) {
		...
		ret = snd_soc_write(codec, reg + i, value);
		if (ret) {
			dev_err(codec->dev,
			 ...
			 struct snd_soc_dapm_widget *sink)
{
	...
	value = snd_soc_read(codec, NAU8822_REG_CLOCKING);
				 ...
				 unsigned int freq, int dir)
{
	...
	nau8822->sysclk = freq;
	dev_info(codec->dev, "master sysclk %dHz, source %s\n", freq,
...
static int nau8822_config_clkdiv(struct snd_soc_dai *dai, int div, int rate)
{
	...
	case NAU8822_CLK_MCLK:
		...
		dev_info(codec->dev, "master clock prescaler %x for fs %d\n",
			div, rate);
				 ...
				 enum snd_soc_bias_level level)
{
	...
	case SND_SOC_BIAS_OFF:
		snd_soc_write(codec, NAU8822_REG_POWER_MANAGEMENT_1, 0);
		snd_soc_write(codec, NAU8822_REG_POWER_MANAGEMENT_2, 0);
		snd_soc_write(codec, NAU8822_REG_POWER_MANAGEMENT_3, 0);
		break;
	}
...
static const struct snd_soc_codec_driver soc_codec_dev_nau8822 = {
	.probe				= nau8822_probe,
			    ...
			    const struct i2c_device_id *id)
{
	...
	ret = snd_soc_register_codec(dev, &soc_codec_dev_nau8822,
						&nau8822_dai, 1);
	if (ret != 0) {
