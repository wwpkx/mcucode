...
int main(void)
{
  ...
  while (1)
  {
    // 读取备份寄存器中的日期值，放到DateToUpdate变量中
    ...
    // 将DateToUpdate变量中最新的日期，保存到备份寄存器中
...
void read_stored_date(RTC_HandleTypeDef * hrtc)
{
  // 1. 先判断DR1（标志位）是否有值，如果等于0说明没有保存过日期
  if (HAL_RTCEx_BKUPRead(hrtc, RTC_BKP_DR1) == 0)
  {
    return;
  }
  
  // 2. 如果DR1有值，说明保存过日期，依次进行读取，赋值给 DateToUpdate
